...
describe('Test the LocationSelector component behavior', () => {
  ...
  it('Automatically selects all beds when "selectAllBedsByDefault" is enabled', () => {
    const readySpy = cy.spy().as('readySpy');
    ...
    cy.get('[data-cy="selector-input"]').select('ALF');
    cy.get('[data-cy="picker-options"]').find('input').should('be.checked');
  });

  it('Automatically selects all beds when switching from a location with no beds to one with beds', () => {
    const readySpy = cy.spy().as('readySpy');
  ...
  it('Automatically selects all beds when switching between two locations with beds', () => {
    const readySpy = cy.spy().as('readySpy');
